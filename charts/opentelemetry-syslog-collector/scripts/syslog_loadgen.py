...
PROFILES = {
    ...
    "switch": {
        "hostnames": [
            "sw-core-01.dc1.local",
            "sw-core-02.dc1.local",
            "sw-access-01.dc1.local",
        ],
        "program": "nos",
        "facility": "local0",
        "format": "rfc5424",
        ...
        "templates": [
            ...
            {
                "msg": "BGP neighbor {peer_ip} state changed from Established to Idle (hold timer expired)",
                "sd": "-",
            },
            {
                "msg": "BGP neighbor {peer_ip} state changed from Idle to Established",
                "sd": "-",
            },
            {
                "msg": "STP topology change detected on port {iface}, transitioning to forwarding",
                "sd": "-",
            },
            {
                "msg": "Port {iface} security violation: MAC address {mac} not allowed",
                "sd": "-",
            },
        ],
    },
    ...
    "postfix": {
        ...
        "templates": [
            {
                "msg": "{queue_id}: to=<{to_addr}>, relay={relay_host}[{relay_ip}]:25, delay={delay}, delays={delays}, dsn=2.0.0, status=sent (250 2.0.0 Ok: queued as {remote_queue_id})",
                "program": "postfix/smtp",
            },
            {
                "msg": "{queue_id}: from=<{from_addr}>, size={mail_size}, nrcpt=1 (queue active)",
                "program": "postfix/qmgr",
            },
            {
                "msg": "{queue_id}: message-id=<{msg_id}@{mail_domain}>",
                "program": "postfix/cleanup",
            },
            {"msg": "connect from {client_host}[{src_ip}]", "program": "postfix/smtpd"},
            {
                "msg": "disconnect from {client_host}[{src_ip}] ehlo=1 mail=1 rcpt=1 data=1 quit=1 commands=5",
                "program": "postfix/smtpd",
            },
            {
                "msg": "{queue_id}: to=<{to_addr}>, relay=none, delay={delay}, delays={delays}, dsn=4.4.1, status=deferred (connect to {relay_host}[{relay_ip}]:25: Connection timed out)",
                "program": "postfix/smtp",
            },
        ],
    },
...
INTERFACES = [f"Gi0/{i}" for i in range(1, 49)] + [
    "Gi1/0/1",
    "Gi1/0/2",
    "Te0/1",
    "Te0/2",
    ...
    def connect(self):
        while not _shutdown:
            try:
                ...
                logger.warning(
                    "TCP connect failed (%s), retrying in %ds", e, self.backoff
                )
...
def main():
    ...
    parser.add_argument(
        "--transport", choices=["tcp", "udp"], default="tcp", help="Transport protocol"
    )
    ...
    logger.info(
        "Starting syslog-loadgen: rate=%.1f msg/s, target=%s:%d/%s",
        args.rate,
        args.host,
        args.port,
        args.transport,
